import { CoachInsight, MatchConfig, MoveEvent } from "./types";
...
export async function requestCoachInsight(params: CoachParams): Promise<CoachInsight> {
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a chess coach. Analyze style and practical mistakes from move history. Keep it concise and actionable with bullet points."
      },
      {
        role: "user",
        content:
          `Give coaching feedback after ply ${params.ply}.\nCurrent FEN: ${params.fen}\nRecent moves:\n${recent || "(none)"}\n\nOutput format:\n1) Style notes\n2) Tactical warning\n3) One concrete tuning idea for evaluation/search.`
      }
    ]
  };
