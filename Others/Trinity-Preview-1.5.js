/**
 * Trinity-Preview-1.5 - Search Refinements
 * Improvements over 1.4:
 * - Enhanced aspiration windows with dynamic adjustment
 * - Improved move ordering with killer from multiple depths
 * - Better time management
 * - Reduced pruning in PV nodes
 * - Enhanced LMR with history weighting
 */
...
function search(depth, alpha, beta, is_pv) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (legal === 1) score = -search(depth - 1, -beta, -alpha, is_pv);
            ...
            // History-weighted LMR
            let hist_score = history[((m & 127) << 7) | ((m >> 7) & 127)];
            if (hist_score > 5000 && reduction > 0) reduction--;
...
function search_root() {
    ...
    for (let d = 1; d <= 64; d++) {
        ...
        if (d >= 3) {
            let window = 20 + d * 3;
            if (Math.abs(prev_score) > 10000) window = 500;
            alpha = Math.max(-50000, prev_score - window);
            beta = Math.min(50000, prev_score + window);
        }
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (best_score > 30000 || best_score < -30000) break;
        }

        // Re-search with wider window if we failed high/low
        if (!stop_search && (best_score <= alpha || best_score >= beta) && Math.abs(best_score) < 30000) {
            alpha = -50000;
            beta = 50000;
            for (let i = 0; i < count; i++) {
                let m = move_stack[i];
                if (!make_move(m)) continue;
                let score = -search(d - 1, -beta, -alpha, true);
                unmake_move(m);
                if (stop_search) break;
                if (score > best_score) { best_score = score; current_best_move = m; }
                if (score > alpha) alpha = score;
        ...
        // Time check for next depth
        if (Date.now() >= stop_time - 100) break;
    }
