...
# ==============================================================================
# PIECE-SQUARE TABLES (PeSTO - High Resolution)
# ==============================================================================
PST_MG, PST_EG = [[0]*128 for _ in range(13)], [[0]*128 for _ in range(13)]
RAW_MG = {
...
Z_SIDE, Z_CASTLE, Z_EP = random.getrandbits(64), [random.getrandbits(64) for _ in range(16)], [random.getrandbits(64) for _ in range(128)]

def zobrist(board, side, castle, ep):
    h = 0
    for sq in range(128):
        if not (sq & 0x88) and board[sq]: h ^= Z_PIECE[board[sq]][sq]
    if side == 'b': h ^= Z_SIDE
    h ^= Z_CASTLE[castle]
    if ep != -1: h ^= Z_EP[ep]
    return h
...
class Trinity:
    def __init__(self, fen):
        self.b = [0]*128
        self.parse_fen(fen)
        self.nodes = self.start = self.abort = 0
        self.limit = 4.85
    ...
    def parse_fen(self, fen):
        ...
        for c in p[0]:
            ...
            else: self.b[r*16+f] = PIECES.index(c); f += 1
        self.side = p[1]
        self.castle = sum(1<<i for i,c in enumerate('KQkq') if c in p[2]) if len(p)>2 else 0
    ...
    def attacked(self, sq, white):
        ...
        for d in (-17,-15) if white else (17,15):
            if not (sq+d & 0x88) and self.b[sq+d] == (WP if white else BP): return True
        # Knights
        for d in (-33,-31,-18,-14,14,18,31,33):
            if not (sq+d & 0x88) and self.b[sq+d] == (WN if white else BN): return True
        # King
        for d in (-17,-16,-15,-1,1,15,16,17):
            if not (sq+d & 0x88) and self.b[sq+d] == (WK if white else BK): return True
        # Sliders
        for dirs, types in [((-16,16,-1,1),'RQ'),((-17,-15,15,17),'BQ')]:
            for d in dirs:
                ...
                while not (c & 0x88):
                    p = self.b[c]
                    if p:
                        if PIECES[p].upper() in types and (p<=6) == white: return True
                        break
                    c += d
        return False
            ...
            if pt == WP:
                ...
                if not caps_only:
                    ...
                    if not (to&0x88) and not self.b[to]:
                        if (to>>4)==pr:
                            for promo in (WQ,WR,WB,WN) if is_w else (BQ,BR,BB,BN): moves.append((sq,to,promo))
                        else:
                            moves.append((sq,to,0))
                            if (sq>>4)==sr and not self.b[to+fwd]: moves.append((sq,to+fwd,0))
                # Captures
                for cd in (fwd-1,fwd+1):
                    ...
                    if not (to&0x88):
                        ...
                        if t and (t<=6)!=is_w:
                            if (to>>4)==pr:
                                for promo in (WQ,WR,WB,WN) if is_w else (BQ,BR,BB,BN): moves.append((sq,to,promo))
                            else: moves.append((sq,to,0))
                        elif to == self.ep: moves.append((sq,to,0))
            elif pt in (WN,WK):
                for d in ((-33,-31,-18,-14,14,18,31,33) if pt==WN else (-17,-16,-15,-1,1,15,16,17)):
                    ...
                    if not (to&0x88):
                        t = self.b[to]
                        if not t: 
                            if not caps_only: moves.append((sq,to,0))
                        elif (t<=6)!=is_w: moves.append((sq,to,0))
                if pt==WK and not caps_only:
                    if is_w:
                        if self.castle&1 and not self.b[5] and not self.b[6] and self.b[7]==WR and not self.attacked(4,0) and not self.attacked(5,0): moves.append((4,6,0))
                        if self.castle&2 and not self.b[1] and not self.b[2] and not self.b[3] and self.b[0]==WR and not self.attacked(4,0) and not self.attacked(3,0): moves.append((4,2,0))
                    else:
                        if self.castle&4 and not self.b[117] and not self.b[118] and self.b[119]==BR and not self.attacked(116,1) and not self.attacked(117,1): moves.append((116,118,0))
                        if self.castle&8 and not self.b[113] and not self.b[114] and not self.b[115] and self.b[112]==BR and not self.attacked(116,1) and not self.attacked(115,1): moves.append((116,114,0))
            elif pt in (WB,WR,WQ):
                dirs = []
                if pt in (WB,WQ): dirs += (-17,-15,15,17)
                if pt in (WR,WQ): dirs += (-16,-1,1,16)
                for d in dirs:
                    to = sq + d
                    while not (to&0x88):
                        t = self.b[to]
                        if not t:
                            ...
                            if (t<=6)!=is_w: moves.append((sq,to,0))
                            break
                        to += d
        return moves
    ...
    def make(self, m):
        ...
        # Castling rook moves
        if p==WK and f==4:
            if t==6: self.b[5],self.b[7]=WR,0; self.hash^=Z_PIECE[WR][5]^Z_PIECE[WR][7]
            elif t==2: self.b[3],self.b[0]=WR,0; self.hash^=Z_PIECE[WR][3]^Z_PIECE[WR][0]
            self.castle &= ~3
        elif p==BK and f==116:
            if t==118: self.b[117],self.b[119]=BR,0; self.hash^=Z_PIECE[BR][117]^Z_PIECE[BR][119]
            elif t==114: self.b[115],self.b[112]=BR,0; self.hash^=Z_PIECE[BR][115]^Z_PIECE[BR][112]
            self.castle &= ~12
        # Update castling rights
        oc = self.castle
        if f in (0,4) or t in (0,4): self.castle &= ~2
        if f in (7,4) or t in (7,4): self.castle &= ~1
        if f in (112,116) or t in (112,116): self.castle &= ~8
        if f in (119,116) or t in (119,116): self.castle &= ~4
    ...
    def unmake(self, m, u):
        ...
        self.side = 'b' if self.side=='w' else 'w'
        p = self.b[t]
        if pr: p = WP if self.side=='w' else BP
        self.b[f], self.b[t] = p, cap
        if p in (WP,BP) and t==ep:
            ex = t - (16 if p==WP else -16)
            self.b[ex] = BP if self.side=='w' else WP
        if p==WK and f==4:
            if t==6: self.b[5],self.b[7]=0,WR
            elif t==2: self.b[3],self.b[0]=0,WR
        elif p==BK and f==116:
            if t==118: self.b[117],self.b[119]=0,BR
            elif t==114: self.b[115],self.b[112]=0,BR
    ...
    def eval_pawns(self):
        ...
        for side_p, enemy_p, sign in [(WP,BP,1),(BP,WP,-1)]:
            ...
            for p in pawns:
                f,r = p&7, p>>4
                if files[f] > 1: score -= 15 * sign  # Doubled
    ...
    def eval_king_safety(self, side):
        ...
        if 0<=shield<8:
            for ff in (f-1,f,f+1):
                if 0<=ff<8 and self.b[shield*16+ff]==(WP if is_w else BP): bonus += 12
        # Penalty for enemy attackers near king
    ...
    def eval_mobility(self, side):
        ...
        for sq in range(128):
            ...
            if pt==WN:
                for d in (-33,-31,-18,-14,14,18,31,33):
                    if not(sq+d&0x88) and not self.b[sq+d]: score += 3
            elif pt in (WB,WR,WQ):
                dirs = []
                if pt in (WB,WQ): dirs += (-17,-15,15,17)
                if pt in (WR,WQ): dirs += (-16,-1,1,16)
    ...
    def evaluate(self):
        ...
        for sq in range(128):
            ...
            if pt==WK: wk = sq if p<=6 else sq
            elif pt==BK: bk = sq if p>6 else sq
        ...
        for sq in range(128):
            if not(sq&0x88):
                if self.b[sq]==WB: wb+=1
                elif self.b[sq]==BB: bb+=1
        if wb>=2: mg+=30; eg+=45
        if bb>=2: mg-=30; eg-=45
        # Add advanced terms
    ...
    def score_move(self, m, tt_m, ply, last):
        f,t,pr = m
        if m==tt_m: return 10**7
        cap = self.b[t]
        if cap:  # MVV-LVA
            return 9*10**6 + MG_VAL[cap if cap<=6 else cap-6]*10 - MG_VAL[self.b[f] if self.b[f]<=6 else self.b[f]-6]
        if pr: return 8*10**6
        if KILLER[ply][0]==m: return 7*10**6
        if KILLER[ply][1]==m: return 6*10**6
        if last and COUNTER[last[0]][last[1]]==m: return 5*10**6
        return HIST[f][t]
        
    def quiesce(self, alpha, beta):
        global NODES, ABORT
        NODES += 1
        if NODES&2047==0 and time.time()-self.start>self.limit: ABORT=True; return 0
        stand = self.evaluate()
        if stand>=beta: return beta
        if stand>alpha: alpha = stand
        moves = [m for m in self.gen_moves(caps_only=True) if self.b[m[1]] or m[1]==self.ep]
    ...
    def search(self, depth, alpha, beta, ply, last, can_null=True):
        ...
        if entry and entry[0]>=depth:
            d,f,v,m = entry
            if f==0: return v
            if f==1 and v<=alpha: return v
            if f==2 and v>=beta: return v
            tt_m = m
        if depth<=0: return self.quiesce(alpha, beta)
        # Razoring
        if depth==1 and self.evaluate()+250<alpha: return self.quiesce(alpha, beta)
        # Null move pruning
        if can_null and depth>=3 and not self.attacked(next((i for i in range(128) if not(i&0x88) and self.b[i]==(WK if self.side=='w' else BK)),-1), self.side=='b'):
            self.side = 'b' if self.side=='w' else 'w'
            self.hash ^= Z_SIDE
            s = -self.search(depth-3, -beta, -beta+1, ply+1, None, False)
            self.side = 'b' if self.side=='w' else 'w'
            self.hash ^= Z_SIDE
            if s>=beta: return beta
        ...
        for i,m in enumerate(moves):
            # Futility pruning
            if depth==1 and i>5 and self.evaluate()+150<alpha and not self.b[m[1]]: continue
            u = self.make(m)
            # Legality
            ks = next((i for i in range(128) if not(i&0x88) and self.b[i]==(WK if self.side=='b' else BK)), -1)
            if self.attacked(ks, self.side=='w'): self.unmake(m,u); continue
            ...
            if i==0:
                ...
                red = 1 if depth>=3 and i>=4 and not self.b[m[1]] else 0
                s = -self.search(depth-1-red, -alpha-1, -alpha, ply+1, m)
                if s>alpha and red: s = -self.search(depth-1, -alpha-1, -alpha, ply+1, m)
                if s>alpha: s = -self.search(depth-1, -beta, -alpha, ply+1, m)
            self.unmake(m,u)
            if ABORT: return 0
            if s>best: best,best_m = s,m
            if s>alpha:
                alpha = s
                if alpha>=beta:
                    if not self.b[m[1]]:  # Quiet move
                        HIST[m[0]][m[1]] += depth*depth
                        KILLER[ply][1],KILLER[ply][0] = KILLER[ply][0],m
                        if last: COUNTER[last[0]][last[1]] = m
                    break
        # Store in TT
    ...
    def get_move(self, fen):
        global TT, HIST, KILLER, COUNTER, NODES, ABORT
        ...
        for depth in range(1, 30):
            if time.time()-self.start>self.limit: break
            val = self.search(depth, alpha, beta, 0, None)
            if ABORT: break
