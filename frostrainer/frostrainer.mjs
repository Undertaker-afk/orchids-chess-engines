...
function printUsage() {
  ...
  console.log(`FrostTrainer

Usage:
  node frostrainer/frostrainer.mjs --student <engine> --opponent <engine> [--opponent <engine> ...] [options]
...
Optional:
  ...
  --timeout-ms <ms>            Student/opponent move timeout. Default: 5000
  ...
  --out-dir <dir>              Output directory. Default: frostrainer/out
...
function resolveEngineSpec(input, baseDir) {
  ...
  if ((raw === 'stockfish' || raw === 'default-coach') && existsSync(DEFAULT_COACH)) raw = DEFAULT_COACH;
...
function parseArgs(argv) {
  const options = {
    ...
    maxPly: 200,
    startFen: START_FEN,
    outDir: resolve(process.cwd(), 'frostrainer', 'out'),
    prefix: '',
  };
...
function buildMarkdown(report) {
  const lines = [];
  lines.push('# FrostTrainer Report');
  lines.push('');
