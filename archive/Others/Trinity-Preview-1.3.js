/**
 * Trinity-Preview-1.3 - Advanced Evaluation Engine
 * Improvements over 1.2:
 * - Bishop pair bonus
 * - Rook on open/semi-open file evaluation
 * - King tropism
 * - Better knight outpost evaluation
 * - Connected rook bonus
 * - Threat detection in evaluation
 */
...
function eval_knights() {
    ...
    for (let color of [WHITE, BLACK]) {
        ...
        for (let sq = 0; sq < 128; sq++) {
            if (!(sq & 0x88) && board[sq] === (KNIGHT | color)) {
                const r = sq >> 4;
                // Knight outpost on enemy territory
                const outpost_rank = color === WHITE ? (r >= 4 && r <= 5) : (r >= 2 && r <= 3);
                if (outpost_rank) {
                    let supported = true;
                    const support_dir = color === WHITE ? 16 : -16;
                    const support_sq = sq + support_dir;
                    if (!(support_sq & 0x88) && board[support_sq] === our_pawn) {
                        // Check if can be attacked by enemy pawn
                        for (let d of (color === WHITE ? [-15, -17] : [15, 17])) {
                            const atk_sq = sq + d;
                            if (!(atk_sq & 0x88) && board[atk_sq] === enemy_pawn) {
                                supported = false;
                                break;
                            }
                        }
                        if (supported) score += 20 * sign;
                    }
                }
            }
        }
