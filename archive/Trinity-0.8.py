...
class Trinity:
    ...
    def see(self, m):
        """Static Exchange Evaluation - simplified but effective"""
        f,t,pr = m
        cap = self.b[t]
    ...
    def quiesce(self, alpha, beta):
        global NODES, ABORT
        NODES += 1
        if NODES&2047==0 and time.time()-START>LIMIT: ABORT=True; return 0
        stand = self.evaluate()
        if stand>=beta: return beta
        if stand>alpha: alpha = stand
        # Only SEE-positive captures to avoid losing trades
    ...
    def search(self, depth, alpha, beta, ply, last, can_null=True):
        ...
        if entry and entry[0]>=depth:
            ...
            if f==1 and v<=alpha: return v  # Upper bound
            if f==2 and v>=beta: return v  # Lower bound
            tt_m = m
        if depth<=0: return self.quiesce(alpha, beta)
        # Razoring: if eval is very low, skip to quiescence
        if depth==1 and self.evaluate()+250<alpha: return self.quiesce(alpha, beta)
        # Null move pruning (avoid in zugzwang positions)
        ...
        for i,m in enumerate(moves):
            ...
            if i==0:
                ...
                s = -self.search(depth-1-reduction, -alpha-1, -alpha, ply+1, m)
                if s>alpha and reduction: s = -self.search(depth-1, -alpha-1, -alpha, ply+1, m)
            ...
            if s>alpha:
                alpha = s
                if alpha>=beta:
                    # Update heuristics on beta cutoff (quiet moves only)
    ...
    def get_move(self, fen):
        global TT, HIST, KILLER, COUNTER, NODES, ABORT, START, LIMIT
        # Memory management: clear TT if too large
        if len(TT)>500000: TT.clear()
        # Reset per-search heuristics (keep TT for cross-move learning)
        HIST = [[0]*128 for _ in range(128)]
        KILLER = [[None,None] for _ in range(256)]
        COUNTER = [[None]*128 for _ in range(128)]
        NODES = ABORT = 0
        self.parse_fen(fen)
        START = time.time()
        LIMIT = 4.85  # Leave buffer for I/O
